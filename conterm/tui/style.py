...
class Color:
    ...
    def new(color: ColorFormat) -> str:
        ...
        elif isinstance(color, str) and color in ["black", "red", "green", "yellow", "blue", "magenta", "cyan", "white"]:
...
class Style:
    ...
    def __init__(self, *styles: S, fg: str|None = None, bg: str|None = None) -> None:
    ...
    def __hash__(self) -> int:
        return self.style + (1 if self.fg != '' else 0) << 16 + (1 if self.bg != '' else 0) << 17
    ...
    # Strike = 9
        ...
        while i < len(codes):
            ...
            if code < 10 and code > 0 and code != 8:
                ...
                if i+1 >= len(codes):
                    raise ValueError(f"Missing special ansi color sequence type <{code};\x1b[33m<2|5>\x1b[39m")
                next = codes[i+1]
                i += 1

                if next != 5 and next != 2:
                    raise ValueError(f"Invalid special ansi color sequence <{code};\x1b[31m{next}\x1b[39m>")
                ...
                if next == 2:
                    if (missing := (i+4) - len(codes)) > 0:
                        ...
                        hint = ';'.join(hint)
                        raise ValueError(f"Missing special ansi color sequence rgb values <{code};2;{hint}>")
                    rgb = codes[i+1:i+4]
                    rest = f"2;{';'.join(str(v) for v in rgb)}"
                    i += 3
                else:
                    if i+1 >= len(codes):
                        raise ValueError(f"Missing special ansi color sequence xterm code <{code};5;\x1b[33m<code>\x1b[39m")
                    rest = f"5;{codes[i+1]}"
                    i += 1
    ...
    @cache
    def reset(self) -> str:
        styles = [Ansi[f'U_{style.name}'] for style in S if self.style & style.value > 0]
        fg, bg = '', ''
        if self.fg != '':
            fg += "39";
        if self.bg != '':
    ...
    def __eq__(self, __value: object) -> bool:
        if isinstance(__value, Style):
            return self.style == __value.style and self.fg == __value.fg and self.bg == __value.bg
        return False
